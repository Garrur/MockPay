...
function GlowButton({
  ...
  variant?: "white" | "orange" | "green" | "red" | "outline";
  className?: string;
}) {
  const [ripples, setRipples] = useState<{ x: number; y: number; id: number }[]>([]);
  const id = useRef(0);
  const styles: Record<string, string> = {
    white: "bg-white text-black hover:bg-white/90 shadow-[0_10px_40px_rgba(255,220,180,0.1)]",
    orange: "bg-orange-700 text-white hover:bg-orange-600 shadow-[0_10px_40px_rgba(194,65,12,0.35)]",
    green: "bg-emerald-600 text-white hover:bg-emerald-500 shadow-[0_10px_40px_rgba(5,150,105,0.4)]",
    red: "bg-red-600 text-white hover:bg-red-500 shadow-[0_10px_40px_rgba(185,28,28,0.35)]",
...
function CardForm() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="text-xs text-stone-300 font-bold mb-1.5 block">Card Number</label>
        <div className="relative">
          <Input value={num} onChange={(e) => setNum(fmt(e.target.value))}
            placeholder="4000 0000 0000 0000"
            className="bg-white/5 border-white/10 text-white pl-12 h-12 focus:border-orange-500/50 font-mono text-sm transition-all" />
          <CreditCard className="absolute left-4 top-3.5 w-5 h-5 text-stone-400" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-stone-300 font-bold mb-1.5 block">Expiry</label>
          <Input placeholder="MM / YY" maxLength={5}
            className="bg-white/5 border-white/10 text-white h-12 font-mono text-sm" />
        </div>
        <div>
          <label className="text-xs text-stone-300 font-bold mb-1.5 block">CVC</label>
          <Input placeholder="•••" type="password" maxLength={3}
            className="bg-white/5 border-white/10 text-white h-12 font-mono text-sm" />
        </div>
      </div>
      <div>
        <label className="text-xs text-stone-300 font-bold mb-1.5 block">Cardholder Name</label>
...
function UpiForm() {
  ...
  const providers = [
    { name: "GPay", emoji: "💸", color: "from-orange-700 to-amber-600" },
    { name: "PhonePe", emoji: "📱", color: "from-amber-700 to-amber-500" },
    { name: "Paytm", emoji: "🏦", color: "from-orange-800 to-orange-600" },
  ];
  return (
    <div className="space-y-5">
      <div>
        <label className="text-xs text-stone-300 font-semibold mb-1.5 block">UPI ID</label>
        <div className="relative">
          <Input placeholder="yourname@upi"
            className="bg-white/5 border-white/10 text-white pl-12 h-12 focus:border-orange-500/50 font-mono text-sm" />
        ...
        <div className="grid grid-cols-3 gap-2">
          {providers.map((p) => (
            <button key={p.name} onClick={() => setSelected(p.name)}
              className={`flex flex-col items-center gap-2 py-3 rounded-xl border transition-all ${selected === p.name ? "border-orange-500/50 bg-orange-500/10" : "border-white/5 bg-white/3 hover:border-white/10"}`}>
              <span className="text-2xl">{p.emoji}</span>
              <span className="text-[10px] font-bold text-stone-300">{p.name}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-xl mx-auto">
        <div className="bg-[#0f0f18]/95 backdrop-blur-xl border border-white/10 rounded-t-3xl shadow-[0_-20px_60px_rgba(0,0,0,0.5)] overflow-hidden">
          <button onClick={() => setOpen((o) => !o)}
            className="w-full h-11 flex items-center justify-center gap-2 text-orange-400 hover:text-orange-300 transition-colors bg-orange-500/5 border-b border-white/5">
          ...
          {open && (
            <div className="p-5 space-y-4">
                  ...
                  <div className="bg-black/40 rounded-xl p-1 flex gap-1 border border-white/5">
                    {[0, 2, 5].map((d) => (
                      <button key={d} onClick={() => setDelay(d)}
                        className={`flex-1 py-2 rounded-lg text-[10px] font-bold transition-all ${delay === d ? "bg-orange-700 text-white shadow-lg" : "text-stone-400 hover:bg-white/5"}`}>
                        {d}s
                      </button>
                    ))}
                  </div>
              ...
              <GlowButton variant="orange" onClick={() => onSimulate(outcome, delay)} disabled={isProcessing}>
...
export default function CheckoutPage({ params }: { params: Promise<{ payment_id: string }> }) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#06060e] flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <div className="w-16 h-16 rounded-2xl bg-orange-600/20 border border-orange-500/20 flex items-center justify-center">
          <Loader2 className="w-7 h-7 text-orange-400 animate-spin" />
        </div>
        <p className="text-stone-300 text-sm animate-pulse font-bold">Securing session...</p>
      </div>
    </div>
  );
  ...
  if (!payment) return (
    <div className="min-h-screen bg-[#06060e] flex flex-col items-center justify-center text-center p-8">
      ...
      <p className="text-stone-300 font-bold max-w-xs mb-8">This payment link is invalid, expired, or has already been processed.</p>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a02] text-white flex items-center justify-center p-4 relative overflow-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <div className="absolute top-[-15%] left-[30%] w-[500px] h-[500px] rounded-full bg-orange-700/8 blur-[100px]" />
        <div className="absolute bottom-[0%] right-[20%] w-[400px] h-[400px] rounded-full bg-amber-600/6 blur-[100px]" />
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {status === "idle" && (
          ...
          >
            <TiltCard className="w-full rounded-[28px] border border-white/[0.05] shadow-[0_50px_150px_rgba(0,0,0,0.9)] overflow-hidden" >
              <div className="grid md:grid-cols-[1fr_1.1fr] bg-[#0e0e0a]/95 backdrop-blur-3xl">
                ...
                <div className="relative p-8 md:p-10 flex flex-col justify-between border-b md:border-b-0 md:border-r border-white/[0.05]">
                  ...
                  <div>
                    ...
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-600 to-orange-800 flex items-center justify-center font-black text-sm shadow-[0_0_20px_rgba(234,88,12,0.3)]">
                        {(payment.project_name || "M")[0].toUpperCase()}
                      </div>
                      <div>
                        <p className="font-bold text-sm leading-tight">{payment.project_name || "MockPay"}</p>
                        <p className="text-[10px] text-orange-400 font-bold uppercase tracking-widest">Demo Mode</p>
                      </div>
                    </div>

                    {/* Amount */}
                    <div className="mb-8">
                      <p className="text-xs text-stone-300 uppercase tracking-widest font-black mb-2">{payment.description || "Order total"}</p>
                    ...
                    <div className="space-y-3 border-t border-white/5 pt-6">
                      <div className="flex justify-between text-sm">
                        <span className="text-stone-300 font-bold">Order ID</span>
                        <span className="font-mono text-stone-300 text-xs">{payment.order_id}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-stone-300 font-bold">Subtotal</span>
                  ...
                  <div className="mt-8 space-y-2">
                    <div className="flex items-center gap-2 text-[10px] text-stone-300 font-bold">
                      <ShieldCheck className="w-3.5 h-3.5 text-green-500/60" />
                      <span>256-bit encrypted · No real money charged</span>
                    </div>
                    <p className="text-[10px] text-gray-700">This is a sandbox simulation. © MockPay 2026.</p>
                  </div>
                </div>
                ...
                <div className="p-8 md:p-10">
                  ...
                  <div className="flex gap-1 bg-black/30 border border-white/5 rounded-2xl p-1 mb-8">
                    {(["card", "upi"] as const).map((t) => (
                      <button key={t} onClick={() => setTab(t)}
                        className={`flex-1 h-11 rounded-xl font-bold text-xs uppercase tracking-tight flex items-center justify-center gap-2 transition-all ${tab === t ? "bg-white/10 text-white shadow-sm" : "text-stone-400 hover:text-stone-200"}`}>
                  ...
                  <div className="mt-8 pt-6 border-t border-white/5 space-y-4">
                    <p className="text-center text-[10px] text-gray-600 leading-relaxed">
                      By clicking Pay, you agree to MockPay's simulated transaction terms.
                    ...
                    <div className="flex justify-center">
                      <a href="/?ref=viral_badge" target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/5 bg-white/3 hover:border-orange-500/30 hover:bg-orange-500/5 transition-all group">
                        <div className="w-4 h-4 rounded bg-orange-600/20 flex items-center justify-center text-[8px] font-black text-orange-400 group-hover:bg-orange-600 group-hover:text-white transition-all animate-pulse group-hover:animate-none">M</div>
                        <span className="text-[10px] text-stone-400 group-hover:text-white transition-colors font-black">Built with MockPay</span>
                      </a>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex justify-center mt-4">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-[10px] font-bold text-amber-500 uppercase tracking-widest">
        ...
        {status === "processing" && (
          ...
          >
            ...
            <div className="relative w-32 h-32 mb-10">
              ...
              <div className="absolute inset-2 rounded-full border border-orange-700/30 animate-spin" style={{ animationDuration: "3s" }} />
              <div className="absolute inset-4 rounded-full border-2 border-t-orange-500 border-orange-500/10 animate-spin" />
              <div className="absolute inset-8 rounded-full border-2 border-orange-400/20 animate-ping" />
              <div className="absolute inset-0 flex items-center justify-center">
                <Zap className="w-8 h-8 text-orange-500" />
              </div>
            </div>
            <h2 className="text-3xl font-black text-white mb-3 tracking-tight">Processing</h2>
            <p className="text-stone-400 max-w-xs leading-relaxed mb-4">Connecting to secure simulated networks. Firing webhook events in real-time.</p>
            {simParams?.delay > 0 && (
              <div className="px-4 py-2 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-400 text-xs font-mono uppercase tracking-widest">
            ...
            <div className="bg-[#0c0c14]/90 backdrop-blur-2xl rounded-[32px] border overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.7)]"
              ...
              <div className="p-10">
                ...
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}
                  className="text-stone-400 leading-relaxed mb-8 text-sm">
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] text-stone-400 uppercase font-black tracking-widest">Payment ID</span>
                    <button onClick={() => { navigator.clipboard.writeText(payment_id); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
                      className="font-mono text-xs text-orange-500 hover:text-orange-400 flex items-center gap-1.5 transition-colors">
                      {copied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                      {copied ? "Copied!" : payment_id.slice(0, 20) + "..."}
                    </button>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] text-stone-400 uppercase font-black tracking-widest">Amount</span>
                    <span className="font-bold">{amount}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] text-stone-400 uppercase font-black tracking-widest">Status</span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-center">
                    <a href="/?ref=viral_badge" target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/5 hover:border-orange-500/30 hover:bg-orange-500/5 transition-all group">
                      <span className="text-[10px] text-gray-600 group-hover:text-orange-500 font-medium transition-colors">⚡ Built with MockPay</span>
                    </a>
                  </div>
                </div>
              </div>
