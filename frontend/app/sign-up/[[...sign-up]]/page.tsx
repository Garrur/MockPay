...
export default function SignUpPage() {
  ...
  // Safely observe the authentication signal for changes
  useEffect(() => {
    if (signUp?.status === "complete") {
      // Once verification successfully advances the status, finalize sets the local active session
      signUp.finalize().then(() => {
        router.push("/dashboard");
      }).catch(err => console.error("Finalize error:", err));
    }
  }, [signUp?.status, router, signUp]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isLoaded || !signUp) {
      alert("Clerk is not fully loaded yet. Please wait.");
      return;
    }
    setError("");
    setLoading(true);
    ...
    try {
      ...
      // Auto-generate a safe username to fulfill Clerk's backend requirements
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (verifyError) {
        ...
        const errorCode = verifyError.code?.toLowerCase() || "";
        const legacyMsg = (verifyError as any).errors?.[0]?.message?.toLowerCase() || "";

        if (errorMsg.includes("already verified") || 
            errorCode.includes("already_verified") ||
            legacyMsg.includes("already verified")) {
           if (signUp.status === "complete") {
  ...
  const handleOAuth = async (strategy: "oauth_google" | "oauth_github") => {
    ...
    try {
      ...
      const msg = err?.errors?.[0]?.longMessage || err?.errors?.[0]?.message || err?.message || "OAuth sign-up failed. Please try again.";
      setError(msg);
    ...
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        <div className="auth-header">
          ...
          <p className="auth-subtitle">Start accepting payments in minutes</p>
        </div>
